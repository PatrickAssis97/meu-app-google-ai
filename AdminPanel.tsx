...
const AdminPanel: React.FC<AdminPanelProps> = (props) => {
  ...
  const { currentUser, onLogout, onViewMural } = props;
    ...
    <div className="p-4 md:p-8 text-white min-h-screen">
      <header className="flex flex-wrap justify-between items-center mb-6 pb-4 border-b-2 border-white/20 gap-4">
        ...
        <div className="flex items-center gap-2">
            <button onClick={onViewMural} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition-colors">
                Visualizar Mural
            </button>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingCourse && 'id' in editingCourse ? 'Editar Curso' : 'Adicionar Curso'}>
                {editingCourse && (
                    <form onSubmit={handleSave} className="space-y-4">
                        <input type="text" placeholder="Nome do Curso" value={editingCourse.name} onChange={e => setEditingCourse({...editingCourse, name: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <input type="number" placeholder="Carga Horária" value={editingCourse.workload} onChange={e => setEditingCourse({...editingCourse, workload: parseInt(e.target.value) || 0})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <input type="text" placeholder="Início (DD/MM/AAAA)" value={editingCourse.startDate} onChange={e => setEditingCourse({...editingCourse, startDate: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <input type="text" placeholder="Término (DD/MM/AAAA)" value={editingCourse.endDate} onChange={e => setEditingCourse({...editingCourse, endDate: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <input type="text" placeholder="Horário (HH:mm - HH:mm)" value={editingCourse.schedule} onChange={e => setEditingCourse({...editingCourse, schedule: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <input type="url" placeholder="URL de Inscrição" value={editingCourse.enrollmentUrl} onChange={e => setEditingCourse({...editingCourse, enrollmentUrl: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <select value={editingCourse.vacancies} onChange={e => setEditingCourse({...editingCourse, vacancies: e.target.value as any})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]">
...
const ManageOngoingCourses: React.FC<Pick<AdminPanelProps, 'ongoingCourses' | 'setOngoingCourses'>> = ({ ongoingCourses, setOngoingCourses }) => {
    ...
    const handleScheduleChange = (index: number, field: keyof CourseSchedule, value: string | number) => {
      if(!editingCourse) return;
        const newSchedules = [...editingCourse.schedules];
        newSchedules[index] = { ...newSchedules[index], [field]: value };
        setEditingCourse({ ...editingCourse, schedules: newSchedules });
    };
    
    const addSchedule = () => {
      if(!editingCourse) return;
        setEditingCourse({ ...editingCourse, schedules: [...editingCourse.schedules, blankSchedule] });
    };

    const removeSchedule = (index: number) => {
        if (!editingCourse || editingCourse.schedules.length <= 1) {
             addToast('O curso deve ter pelo menos um horário.', 'error');
            return;
        }
        const newSchedules = editingCourse.schedules.filter((_, i) => i !== index);
        setEditingCourse({ ...editingCourse, schedules: newSchedules });
    };
    ...
    return (
      <>
        ...
        <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingCourse && 'id' in editingCourse ? 'Editar Curso' : 'Adicionar Curso'}>
          {editingCourse && (
            <form onSubmit={handleSave} className="space-y-4">
              <input type="text" placeholder="Nome da Sala (Ex: Sala 101)" value={editingCourse.room} onChange={e => setEditingCourse({...editingCourse, room: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
              <input type="text" placeholder="Nome do Curso" value={editingCourse.courseName} onChange={e => setEditingCourse({...editingCourse, courseName: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Bloco</label>
                <select value={editingCourse.block} onChange={e => setEditingCourse({...editingCourse, block: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]">
...
const ManageNews: React.FC<Pick<AdminPanelProps, 'newsItems' | 'setNewsItems'>> = ({ newsItems, setNewsItems }) => {
    ...
    return (
        <>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingNews && 'id' in editingNews ? 'Editar Notícia' : 'Adicionar Notícia'}>
                {editingNews && (
                    <form onSubmit={handleSave} className="space-y-4">
                        <input type="text" placeholder="Título" value={editingNews.title} onChange={e => setEditingNews({...editingNews, title: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <WysiwygEditor
                            content={editingNews.content}
                            setContent={content => setEditingNews(prev => prev ? { ...prev, content } : null)}
                        />
                        <input type="url" placeholder="URL da Imagem de Fundo (Opcional)" value={editingNews.imageUrl || ''} onChange={e => setEditingNews({...editingNews, imageUrl: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" />
                        <label className="flex items-center gap-2 text-gray-300 cursor-pointer">
                            <input type="checkbox" checked={editingNews.urgent} onChange={e => setEditingNews({...editingNews, urgent: e.target.checked})} className="form-checkbox h-5 w-5 text-[#f58220] bg-gray-700 border-gray-600 rounded focus:ring-[#f58220]"/>
    ...
    return (
        <>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={'Adicionar Novo Usuário'}>
                {editingUser && (
                    <form onSubmit={handleSave} className="space-y-4">
                        <input type="text" placeholder="Nome de usuário" value={editingUser.username} onChange={e => setEditingUser({...editingUser, username: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <input type="password" placeholder="Senha" value={editingUser.password} onChange={e => setEditingUser({...editingUser, password: e.target.value})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" required />
                        <select value={editingUser.role} onChange={e => setEditingUser({...editingUser, role: e.target.value as any})} className="w-full p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]">
...
const ManageSettings: React.FC<Pick<AdminPanelProps, 'settings' | 'setSettings'>> = ({ settings, setSettings }) => {
    ...
    const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setSettings(prev => ({ ...prev, customLogoUrl: reader.result as string }));
                addToast('Nova logo enviada com sucesso!', 'success');
            };
    ...
    const handleRemoveLogo = () => {
        ...
        addToast('Logo personalizada removida.', 'success');
    };
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-black/20 p-4 rounded-lg">
                 ...
                 {/* Footer Settings */}
                 <div className="bg-black/20 p-4 rounded-lg">
                    <h3 className="text-xl font-bold mb-4 border-b border-white/20 pb-2">Rodapé e Logo</h3>
                    <div className="space-y-3">
                        <h4 className="font-semibold">Exibir no Rodapé:</h4>
                        <label className="flex items-center gap-2 text-gray-200 cursor-pointer"><input type="checkbox" checked={settings.footerSettings.showLogo} onChange={e => setSettings(prev => ({...prev, footerSettings: {...prev.footerSettings, showLogo: e.target.checked }}))} className="form-checkbox h-5 w-5 text-[#f58220] bg-gray-700 border-gray-600 rounded focus:ring-[#f58220]"/> Logo</label>
                        <label className="flex items-center gap-2 text-gray-200 cursor-pointer"><input type="checkbox" checked={settings.footerSettings.showClock} onChange={e => setSettings(prev => ({...prev, footerSettings: {...prev.footerSettings, showClock: e.target.checked }}))} className="form-checkbox h-5 w-5 text-[#f58220] bg-gray-700 border-gray-600 rounded focus:ring-[#f58220]"/> Relógio e Data</label>
                    </div>
                     <hr className="border-gray-600 my-4"/>
                     <div className="space-y-3">
                        <h4 className="font-semibold">Logo Institucional:</h4>
                        <input type="file" accept="image/*" ref={logoInputRef} onChange={handleLogoUpload} className="hidden" id="logo-upload" />
                         <div className="flex gap-2">
                             <label htmlFor="logo-upload" className="flex-1 text-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded cursor-pointer transition-colors">
                                Alterar Logo
                            </label>
                            {settings.customLogoUrl && (
                                <button onClick={handleRemoveLogo} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Remover</button>
                            )}
                         </div>
                     </div>
                </div>
